...
def printHelp():
	printVersion()
	print "Usage: zakupki.gov_parser [option]"
	print "Options:"
	print "\t-d, --dbfile\t\t\tpath to database file"
	print "\t-l, --linksfile\t\t\tpath to links file for parsing to table `links`"
	print "\t    --logfile\t\t\tpath to log file"
	print "\t    --exportfile\t\tpath to file for export ( *.xlsx )\n"
	print "\t-a, --action\t\t\taction to perform"
	print "\t-v, --verbose\t\t\tverbose mode\n"
	print "\t    --sleep\t\t\tsleep time between requests"
	print "\t-V, --version\t\t\tprint script version"
	print "\t-h, --help\t\t\tprint help"

def printVersion():
	print "zakupki.gov_parser, version: %s" % ( _version )
...
def main():
	...
	options, remainder = getopt.getopt( sys.argv[1:], 'vVd:a:l:s:h', [
																'verbose',
																'version',
																'dbfile=',
																'linksfile=',
